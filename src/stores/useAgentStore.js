...
const AGENT_REFRESH_TYPES = new Set([
  'agent_count_update', 'agent_update', 'agent_completed',
...
const PANEL_REFRESH_TYPES = new Set([
  'agent_count_update', 'agent_update', 'agent_completed',
  'live_feed_entry', 'workflow_completed', 'workflow_error',
  'workflow_started',
])
...
const INITIAL_AGENTS = AGENTS.map(a => ({
  ...a, active: 0, queued: 0, processed: 0,
...
const useAgentStore = create((set, get) => ({
    ...
    ws.onmessage = (ev) => {
      try {
        ...
        if (type === 'header_refresh' || type === 'initial_state') {
          ...
          return  // no REST fetch needed — data came inline
        }
  ...
  disconnectWs: () => {
    ...
    if (_ws) _ws.close()
    set({ _ws: null, _retryTimer: null, _fetchAgentsTimer: null, _fetchPanelTimer: null, wsConnected: false })
  },
